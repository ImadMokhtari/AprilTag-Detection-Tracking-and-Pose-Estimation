    ...
    int main()
    {
        ...
        int detection_thread,tracking_thread;

        detection_thread = pthread_create(&detection, NULL, detectionv, (void*)1);
        if (detection_thread)
        {
            cout << "Error:unable to create thread," << detection_thread << endl;
            exit(-1);
        }

        tracking_thread = pthread_create(&tracking, NULL, trackingv, (void*)1);
        if (tracking_thread) {
            cout << "Error:unable to create thread," << tracking_thread << endl;
            exit(-1);
        }
       ...
       // while(1)
        //{
            ...
            if(detection_begin==0)
            {
                ...
                int if_detection=zarray_size(detections);
                for (int i = 0; i < if_detection; i++)
                {
                 ...
                 while (detec)
                 {
                    ...
                    detections = apriltag_detector_detect(td, &im);
                    if_detection=zarray_size(detections);
                    // Draw detection outlines
                    for (int i = 0; i < if_detection; i++)
                    {
                        //apriltag_detection_t *det;   it was declaratedoutside the loop.
